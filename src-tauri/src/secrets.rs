...
pub fn get_api_key() -> Result<String, String> {
    ...
    std::env::var("OPENAI_API_KEY")
        .map_err(|_| "OPENAI_API_KEY non définie".to_string())
}
...
/// Utilise un RwLock pour permettre lectures concurrentes
pub fn get_api_key_cached() -> Result<String, String> {
    // Tentative lecture cache (lock partagé, très rapide)
    ...
    // Cache miss: récupérer du keychain
    let key = get_api_key()?;
    
    // Mise à jour cache (lock exclusif)
    if let Ok(mut cache) = API_KEY_CACHE.write() {
...
pub fn remove_api_key(key_id: String) -> Result<(), String> {
    ...
    if config.active_key_id == key_id {
        config.active_key_id = config.keys.first()
            .map(|k| k.id.clone())
            .unwrap_or_default();
    }
...
pub async fn test_api_key(key: &str) -> Result<(), String> {
    ...
    let resp = client
        ...
        .map_err(|e| {
            if e.is_timeout() {
                ...
                "Impossible de se connecter à OpenAI. Vérifiez votre connexion internet.".to_string()
            } else {
    ...
    match resp.status() {
        ...
        reqwest::StatusCode::TOO_MANY_REQUESTS => {
            Err("Quota dépassé. Attendez quelques minutes ou vérifiez votre plan OpenAI.".to_string())
        }
        reqwest::StatusCode::SERVICE_UNAVAILABLE => {
            Err("Service OpenAI temporairement indisponible. Réessayez dans quelques instants.".to_string())
        }
        status => {
            Err(format!("Erreur API OpenAI: {} - Vérifiez votre clé.", status))
        }
    }
}
...
mod tests {
    ...
    fn test_validate_openai_key_valid() {
        ...
        assert!(validate_openai_key("sk-proj-1234567890abcdefghijklmnopqrstuvwxyz1234567890").is_ok());
    }
    ...
    fn test_validate_openai_key_invalid_chars() {
        assert!(validate_openai_key("sk-1234567890abcdefghijklmnopqrstuvwxyz1234567890!@#").is_err());
    }
    ...
    fn test_validate_anthropic_key_valid() {
        assert!(validate_anthropic_key("sk-ant-1234567890abcdefghijklmnopqrstuvwxyz1234567890").is_ok());
    }
    ...
    fn test_validate_api_key_router() {
        ...
        // Anthropic
        assert!(validate_api_key("sk-ant-1234567890abcdefghijklmnopqrstuvwxyz", "anthropic").is_ok());
